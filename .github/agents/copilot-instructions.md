# catholic-liturgy-tools Development Guidelines

Auto-generated from all feature plans. Last updated: 2025-11-22

## Active Technologies
- File system (markdown files in `_posts/` directory for Jekyll compatibility) (001-github-pages)
- Python 3.11+ (per constitution Principle 7) (005-daily-reflections)
- File-based HTML generation (existing pattern in `_site/readings/`), no database required (005-daily-reflections)

- Python 3.11 (001-github-pages)

## Project Structure

```text
src/
tests/
```

## Commands

cd src [ONLY COMMANDS FOR ACTIVE TECHNOLOGIES][ONLY COMMANDS FOR ACTIVE TECHNOLOGIES] pytest [ONLY COMMANDS FOR ACTIVE TECHNOLOGIES][ONLY COMMANDS FOR ACTIVE TECHNOLOGIES] ruff check .

## Code Style

Python 3.11: Follow standard conventions

## Recent Changes
- 005-daily-reflections: Added Python 3.11+ (per constitution Principle 7)
- 004-preserve-site-history: Added [if applicable, e.g., PostgreSQL, CoreData, files or N/A]
- 001-github-pages: Added Python 3.11


<!-- MANUAL ADDITIONS START -->
<!-- MANUAL ADDITIONS END -->
